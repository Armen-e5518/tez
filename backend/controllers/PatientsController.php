...
class PatientsController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        $Problems = $model->problems_id ? ['problems_id' => $model->problems_id] : ['problems_id' => 0];
        $searchModelProblems = new ProblemsSearch($Problems);
        $dataProviderProblems = $searchModelProblems->search(Yii::$app->request->queryParams);


        $Conditions = $model->conditions_id ? ['conditions_id' => $model->conditions_id] :['conditions_id' => 0 ];
        $searchModelConditions = new ConditionsSearch($Conditions);
        $dataProviderConditions = $searchModelConditions->search(Yii::$app->request->queryParams);
        return $this->render('view', [
            'model' => $model,
            'searchModelProblems' => $searchModelProblems,
            'dataProviderProblems' => $dataProviderProblems, 
            'searchModelConditions' => $searchModelConditions,
